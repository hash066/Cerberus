...
- **Remaining Phase 1 lanes**: key custody (issuer key → OS keystore/TPM/Secure-Enclave, `daemon/auth/keystore.go` + `core/tee`); LAN wire mTLS + PeerID-pin on `daemon/dataplane`; DoS/quota enforcement on all network surfaces; threat-model doc + external audit. Smaller follow-up: burn down the 70 golangci-lint findings to make that a hard gate.
