...
- **Web dashboard** ✅ (`fd36416`): minimalistic Docker-style static site under `web/` (index/install/docs), wolf logo+favicon, honest beta banners, pairing/Tailscale + security docs grounded in real behavior.
- **One-click daemon (sidecar)** ✅ (`7eac5ce`): `cerberusd` is now bundled into the app (`bundle.externalBin` + `tray/build-sidecar.mjs` builds the triple-named binary in a before-build hook) and auto-spawned from `tray/src-tauri/src/lib.rs` `setup()` (drained; killed on `RunEvent::Exit`). Installers are now ~25 MB (.msi)/~21 MB (.exe). **Verified from a clean slate**: launching the built `tray.exe` spawned exactly one `cerberusd`, `/healthz`=ok, status `mesh_up=true` — zero manual steps.
- **#1 NEXT beta task — the Run button is not wired to a working exec path.** Brutally-honest feature audit found: install/auto-start/status/see-machines/devices/metrics all WORK, but the app's **"Run a workload"** submits to the gateway `/v1/chat/completions` and gets HTTP 200 with **empty content** — the real working execution path is the mesh `RequestCompute` (the e2e demo → 1337), which the app's Run button does NOT use. So a beta tester's first click ("run an agent") lands on the one broken thing. Wire the gateway (or the tray Run command) to the real exec path. Secondary gaps: no in-app "add remote machine" button (cross-network pairing is the `--peer` CLI flag); workloads-history list always empty.
...
### Production hardening — Phase 1 in progress (2026-07-02)
Security core (threat model: buggy/compromised agent code). CI green on `74cfa13`. Landed so far:
- **wazero sandbox** ✅ (`0d8d35e`): adversarial suite + module-loader fuzz prove confinement — ungranted host imports fail closed (no ambient authority), traps don't panic the host, no goroutine leak over 250 mixed runs, loader never panics on garbage. **No wasm.go changes were needed** — the sandbox held.
- **OCap kernel audit** ✅ (`9d3f800`, `74cfa13`): 23 Rust + 18 Go adversarial/property tests (attenuation narrowing, revocation, time bounds, nonce/id uniqueness, signature fail-closed, confused-deputy). Found + fixed **two real bugs**: (1) `SignedCap.Issue` never set a capability id → every Issue-minted cap collided on id 000…0 so revoking one revoked all / none individually; (2) wire-path revocation only cascaded one hop → revoking a root didn't deny a depth-≥2 descendant. The envelope is now v2 carrying the full ancestor chain so revoking any ancestor denies the whole subtree.
- **Also** (`390c5bd`): the fuzz gate caught a capability-envelope decode DoS (4-billion-count → hang/OOM), fixed.
- **Key custody** ✅ (`aa069ba`): issuer key moved into the OS credential store (Windows Credential Manager / macOS Keychain / Linux Secret Service via go-keyring), wired into `cmd/cerberusd` with fail-safe migration of an existing plaintext `issuer.key` (file stays source-of-truth until the seed reads back from the keychain, then the file is deleted). Headless CI → file fallback. Software custody (honest); hardware TEE sealing still the future step.
- **LAN wire mTLS + PeerID-pin** ✅ (`4a5ca86`): mutual TLS on the raw QUIC data plane bound to each node's Ed25519 identity; server records the authenticated client PeerID after the cap/quota gates; dialer pins the server PeerID; missing/non-Ed25519/ wrong-pin all rejected.
- **Remaining Phase 1 lanes**: DoS/quota enforcement on all network surfaces; threat-model doc + external audit. Smaller follow-up: burn down the 70 golangci-lint findings to make that a hard gate. (4 of ~6 security-core lanes done; CI green on `84793bb`, both branches synced.)
...
| 9P2000.L wire server (`daemon/ninep` via hugelgupf/p9) | ✅ REAL, tested (Phase F4) — serves the cap-gated namespace; per-connection capability; `ctl`→endpoint invariant held. **Mount is no longer a stub:** `cerberusd -mount` is a ✅ real FUSE mount on Linux (pure-Go go-fuse → `/dev/fuse`, no cgo; verified on WSL2 — `TestMountLive` reads a device's `info`/`ctl` leaves through the kernel). ⚠️ Windows (cgofuse→WinFsp) exists in code but is **unverified**; ⛔ macOS not shipped. **`/cer/fs` is NOT browsable through a mount** (not enumerated; read-open → `ENOSYS` at `wire.go:256`). |
| Network audio transport — mic/speaker (`daemon/audio`) | ✅ REAL, tested (Phase F5) — packetized sender/receiver, reordering jitter buffer, DLL drift control, gap-fill. **Now rides the data plane:** `daemon/audiolink` carries an audio session as one capability/quota-bound QUIC transfer (packets length-framed by `audio.SendTransport`/`RecvTransport`); `audio` stays a leaf (depends only on its `Transport` interface). Proven end-to-end by `audiolink` `TestAudioRidesDataPlane` (SineSource → QUIC under a grant → reconstructed). **OS capture is REAL** on Windows (WASAPI) and Linux (PulseAudio native protocol, pure Go, no cgo — covers PipeWire via pipewire-pulse). ⛔ macOS CoreAudio: written, **NEVER COMPILED**, behind `darwin && cgo && cerberus_coreaudio`. |
| GPU dispatch (wgpu) | ⛔ Phase F2 — deferred (needs real GPU hardware to validate; not faked). *(The "MLX" half of this row is gone: it was a mock, deleted in `72a0f4a`.)* |
| LLM inference (`daemon/llama`) | ⚠️ **WIRED (new), opt-in** — `cmd/cerberusd/main.go:509`: `-llama-model` starts a real `llama-server` and `gw.SetInference(svc)` makes `/v1/chat/completions` live with llama.cpp's real tokenizer/sampler/`usage`. `-llama-rpc` offloads tensors to peers; `-llama-worker` lends this node's GPU (OFF by default — `ggml-rpc` trusts its client). Without `-llama-model` the daemon logs `0 inference model(s) registered` and the gateway's hello-shard fallback answers any model name (known bug). The v0.1 `llamacpp`/`mlx` backends this replaces were mocks and were deleted, not repaired. **Measured:** RPC-distributed is ~9× slower than single-node — value is fit, not speed. |
| Capability scoping | ✅ REAL in the shipping pure-Go kernel (`stub.go:113`; `ninep/capscope_proof_test.go`) **and at every mesh gate** — `daemon/mesh/capscope.go`'s `grantCoversResource` is applied by gpu/compute/audio/shard/meta/component-fetch/llama-rpc, each with a per-protocol denial test plus prefix-confusion and wildcard rejection. *(This closed a real cross-service substitution hole: compute and gpu both need `exec`, so a compute cap opened the GPU.)* ⚠️ **Residual:** the `-tags ffi` Rust kernel **cannot** scope — its C ABI is `(handle, op, now)` with no resource. Releases are pure Go, so this does not affect them. |
