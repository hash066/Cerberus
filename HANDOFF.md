...
| LLM inference (`daemon/llama`) | ⛔ **WRITTEN, TESTED, NOT WIRED** — supervises real `llama-server`/`ggml-rpc-server` over a cap-gated tunnel, but `grep -r cerberus/daemon/llama` returns **zero importers**, so it does not run. The v0.1 `llamacpp`/`mlx` backends it replaces were mocks and were deleted, not repaired. |
| Capability scoping | ✅ REAL in the shipping pure-Go kernel (`stub.go:113`; `capscope_proof_test.go`). ⚠️ **Residuals:** `daemon/mesh`'s audio/shard/compute/gpu gates verify rights but **never compare `grant.Resource`** (only `llamarpc.go` does); the `-tags ffi` Rust kernel **cannot** scope — its C ABI is `(handle, op, now)` with no resource. |
