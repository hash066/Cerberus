...
| OCap on WASM Component Model | **Shippable** (best-in-class) | Real, with a caveat: caps are resource-scoped in the pure-Go kernel that ships; **four mesh protocols (audio/shard/compute/gpu) verify rights but not scope**, and the `-tags ffi` Rust kernel cannot scope at all (the frozen C ABI takes no resource) |
...
| Real-time very-large-model inference over Wi-Fi | **Frontier** (latency-bound) | **Nothing.** No LLM runs through Cerberus. `daemon/llama` is written, tested, and imported by no binary. Measured out-of-tree with upstream llama.cpp, distributing a model over RPC was **~9× slower** than one node holding it — the value is fitting a model that fits nowhere, never speed |
