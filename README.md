...
| VRAM telemetry: the number the scheduler ranks GPU work on | **Real** and source-tagged — it reads the same `nvidia-smi` a human would, so the two cannot silently disagree. Re-verified on an RTX 3050 Laptop: **4096 MiB total, exactly matching `nvidia-smi`** (free is a live figure and drifts between reads — 3964 vs 3951 MiB seconds apart — which is why `free` is queried, never derived as total−used). A card it cannot measure reports **unknown** (0/0, infeasible for placement), never a plausible default | `daemon/gpu/vramprobe_nvidia.go`, [docs/vram.md](docs/vram.md) |
